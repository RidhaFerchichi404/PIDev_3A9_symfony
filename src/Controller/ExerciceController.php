...
final class ExerciceController extends AbstractController
{
    ...
    {
        // Vérifier que l'utilisateur actuel est le propriétaire de cet exercice
        $user = $this->getUser();
        if (!$user || $exercice->getIdUser() !== $user->getId()) {
            $this->addFlash('error', 'Vous ne pouvez modifier que les exercices que vous avez ajoutés.');
    ...
    public function delete(Request $request, Exercice $exercice, EntityManagerInterface $entityManager): Response
    {
        ...
        $user = $this->getUser();
        if (!$user || $exercice->getIdUser() !== $user->getId()) {
            $this->addFlash('error', 'Vous ne pouvez supprimer que les exercices que vous avez ajoutés.');
            return $this->redirectToRoute('app_exercice_index');
        }
