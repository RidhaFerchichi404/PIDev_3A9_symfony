...
final class EquipementController extends AbstractController
{
    ...
    {
        // Vérifier que l'utilisateur actuel est le propriétaire de cet équipement
        $user = $this->getUser();
        if (!$user || $equipement->getIdUser() !== $user->getId()) {
            $this->addFlash('error', 'Vous ne pouvez modifier que les équipements que vous avez ajoutés.');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // Mettre à jour l'ID de l'utilisateur qui fait la modification
            $user = $this->getUser();
            if ($user) {
                $equipement->setIdUser($user->getId());
                
                // Ajouter un historique si nécessaire
                // ...
            }
            ...
            return $this->redirectToRoute('app_salle_de_sport_equipements', [
                'id' => $equipement->getSalleDeSport()->getId(),
    ...
    public function delete(Request $request, Equipement $equipement, EntityManagerInterface $entityManager): Response
    {
        ...
        $user = $this->getUser();
        if (!$user || $equipement->getIdUser() !== $user->getId()) {
            $this->addFlash('error', 'Vous ne pouvez supprimer que les équipements que vous avez ajoutés.');
            return $this->redirectToRoute('app_equipement_index');
        }
